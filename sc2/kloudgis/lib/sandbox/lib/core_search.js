KG.core_search = SC.Object.create({
	...
	searchFeatures: function(){
		...
		if(content && content.destroy){
			content.forEach(function(cat){store.unloadRecord(KG.SearchCategory, cat.get('id'), cat.get('storeKey'))});
			content.destroy();
		}
            ...
            success: function(data, textStatus, jqXHR) {
				console.log('Google success.');
               	if(data && data.results && data.results.length > 0){
					var results = data.results, i, records = [];				
					for(i=0;i<results.length; i++){
						var geo = {x: results[i].geometry.location.lng, y:results[i].geometry.location.lat};
						records.pushObject(SC.Object.create({title: results[i].formatted_address, coords: [geo]}));
					}
					KG.store.loadRecord(KG.SearchCategory, {category: '*Google*', categoryLabel: 'Google', count: data.results.length, loaded_records: records}, 900913);
				}		
            },
            async: YES
        });
