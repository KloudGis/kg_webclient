...
KG.UploadFileView = SC.View.extend(SC.DelegateSupport,
/** @scope SC.UploadFileView.prototype */
{
	
    classNames: 'kg-file-field-view'.w(),
    ...
    submitForm: function() {
        ...
        if (SC.none(files) || files.length === 0) {
            return;
        }
        ...
        xhr.open("POST", this.get('postUrl'));
        xhr.send(fd);
    ...
    _mouseDownInInput: function(evt) {
        SC.Logger.debug('mouse down input');
        var input = this._input;
    ...
    _mouseUpInInput: function(evt) {
        SC.Logger.debug('mouse up input');
        var input = this._input;
    ...
    _mouseOutOfInput: function(evt) {
        SC.Logger.debug('mouse out input');
        var input = this._input;
        SC.Event.remove(input.$()[0], 'mouseup', this, this._mouseUpInInput);
        SC.Event.remove(input.$()[0], 'mouseout', this, this._mouseOutOfInput);
    ...
    _createForm: function() {
        SC.Logger.debug('create the form');
        var form;
        form = SC.View.create({
            tagName: 'div',

            classNames: 'kg-file-field-form'.w(),
    ...
    _createInput: function() {
        SC.Logger.debug('create the input');
        this._button = SC.ButtonView.create({
            ...
            classNames: 'kg-file-field-button-view'.w(),
        ...
        this._input = SC.View.create(SC.Control, {
            ...
            classNames: 'kg-file-field-input-view'.w(),
