//©2006-2011 Strobe Inc. and contributors.   Portions ©2008-2011 Apple Inc. All rights reserved. Licensed under MIT license// ==========================================================================
...
if (Ember.EXTEND_PROTOTYPES) {
  ...
  */
  Function.prototype.handleActions = function() {
    var args = Array.prototype.slice.call(arguments);
    args.unshift(this);
    return Ember.handleActions.apply(SC, args);
  };
  ...
  */
  Function.prototype.stateObserves = function() {
    var args = Array.prototype.slice.call(arguments);
    args.unshift(this);
    return Ember.stateObserves.apply(SC, args);
  };
}
...
var get = Ember.get, set = Ember.set, getPath = Ember.getPath, slice = Array.prototype.slice;
...
/**
  ...
  For more information on using statecharts, see Ember.StatechartManager.

  @author Michael Cohen
  @extends Ember.Object
*/
Ember.State = Ember.Object.extend(
  /** @lends Ember.State.prototype */ {
  ...
  */
  ...
  /**
    ...
    will be an instance of an empty state (Ember.EmptyState).
  ...
  */
  ...
  /** 
    ...
    @see Ember.StatechartManager#trace
  ...
  */
  ...
  /** 
    ...
    @see Ember.StatechartManager#owner
  ...
  */
  ...
  destroy: function() {
    ...
    if (sc) {
      Ember.removeObserver(sc, ownerKey, this, '_statechartOwnerDidChange');
      Ember.removeObserver(sc, traceKey, this, '_statechartTraceDidChange');
    }
  ...
  */
  initState: function() {
    ...
    if (Ember.HistoryState.detect(initialSubstate) && initialSubstate.isClass) {
      ...
      if (Ember.none(get(historyState, 'defaultState'))) {
    ...
    for (key in this) {
      value = this[key];
      valueIsFunc = Ember.typeOf(value) === "function";
      ...
      if (Ember.State.detect(value) && value.isClass && this[key] !== this.constructor) {
    ...
    if (!Ember.none(initialSubstate) && !matchedInitialSubstate) {
    ...
    if (substates.length === 0) {
      ...
      if (Ember.none(initialSubstate) && !substatesAreConcurrent) {
      ...
      else if (!Ember.none(initialSubstate) && substatesAreConcurrent) {
        set(this, 'initialSubstate', null);
  ...
  */
  createEmptyState: function(attrs) {
    return Ember.EmptyState.create(attrs);
  },
  ...
  */
  _registerStateObserveHandler: function(name, handler) {
    ...
    for (; i < len; i += 1) {
      arg = args[i];
      if (Ember.typeOf(arg) !== "string" || Ember.empty(arg)) { 
  ...
  */
  pathRelativeTo: function(state) {
    ...
    while (!Ember.none(parent) && parent !== state) {
  ...
  */
  getSubstate: function(value) {
    ...
    if (Ember.none(paths)) return null;
    ...
    if (!Ember.none(state)) return state;
  ...
  /**
    ...
    Where <state> is either a string or a Ember.State object and <hash> is a regular JS hash object.
    
    @param state {Ember.State|String} the state to go to
  ...
  */
  ...
  /**
    ...
    @param state {Ember.State|String} the state whose history state to go to
  ...
  */
  stateIsCurrentSubstate: function(state) {
    if (Ember.typeOf(state) === "string") state = get(this, 'statechart').getState(state);
  ...
  reenter: function() {
    ...
    if (get(this, 'isCurrentState')) {
      statechart.gotoState(this);
       ...
       Ember.Logger.error('Can not re-enter state %@ since it is not a current state in the statechart'.fmt(this));
    }
  },
  ...
  */
  tryToHandleAction: function(action, arg1, arg2) {
    ...
    if (Ember.typeOf(this[action]) === "function") {
    ...
    // handle the action
    if (Ember.typeOf(this['unknownAction']) === "function") {
  ...
  _configureAllStateObserveHandlers: function(action) {
    ...
    for (key in this._registeredStateObserveHandlers) {
      ...
      for (i = 0; i < values.length; i += 1) {
        ...
        tuple = Ember.normalizeTuple(this, path);
  ...
  */
  performAsync: function(func, arg1, arg2) {
    return Ember.Async.perform(func, arg1, arg2);
  },
  ...
  */
  respondsToAction: function(action) {
    ...
    if (Ember.typeOf(this[action]) === "function") return true;
    ...
    return Ember.typeOf(this['unknownAction']) === "function";
  },
  ...
  */
  ...
  // toString: function() {
    ...
    // return "%@<%@, %@>".fmt(className, get(this, 'fullPath'), Ember.guidFor(this));
  // },
...
/**
  ...
  Example:
      ...
      MyApp.statechart = Ember.Statechart.create({
        rootState: Ember.State.extend({
          initialSubstate: 'a',
          a: Ember.State.plugin('path.to.a.state.class'),
          b: Ember.State.plugin('path.to.another.state.class')
        })
      });
  ...
  implement required functionality:
      ...
      stateA: Ember.State.plugin('path.to.state', SomeMixin, { ... })

  @param value {String} property path to a state class
  @param args {Hash,...} Optional. Hash objects to be added to the created state
*/
Ember.State.plugin = function(value) {
  var args = slice.call(arguments); args.shift();
  var func = function() {
    var klass = Ember.getPath(window, value);
    if (!klass) {
      console.error('Ember.State.plugin: Unable to determine path %@'.fmt(value));
      return undefined;
    }
    if (!klass.isClass || (klass.isInstance && !(klass instanceof Ember.State))) {
      console.error('Ember.State.plugin: Unable to extend. %@ must be a class extending from Ember.State'.fmt(value));
      return undefined;
    }
...
/**
  ...
  Represents a history state that can be assigned to a Ember.State object's
  initialSubstate property. 
  
  If a Ember.HistoryState object is assigned to a state's initial substate, 
  ...
  An example of how to use:
    ...
    stateA: Ember.State.extend({
      ...
      initialSubstate: Ember.HistoryState({
        defaultState: 'stateB'
      }),
  ...
  @extends Ember.Object
*/
Ember.HistoryState = Ember.Object.extend(
  /** @scope Ember.HistoryState.prototype */{
...
/**
  ...
  The following example shows how states are nested within a statechart:
      ...
      MyApp.Statechart = Ember.Object.extend(Ember.StatechartManager, {
        rootState: Ember.State.extend({
  ...
  states, you can set the rootStateExample property with a class that extends from Ember.State. If the 
  rootStateExample property is not explicitly assigned the then default class used will be Ember.State.
      ...
      MyApp.Statechart = Ember.Object.extend(Ember.StatechartManager, {
        rootState: Ember.State.extend({
          ...
          stateA: Ember.State.extend({
            ...
            stateM: Ember.State.extend({ ... })
            stateN: Ember.State.extend({ ... })
            stateO: Ember.State.extend({ ... })
          }),
        
          stateB: Ember.State.extend({
            initialSubstate: 'stateX',

            stateX: Ember.State.extend({ ... })
            stateY: Ember.State.desgin({ ... })
          })
        })
      });
  ...
  within one file. We could update the code and split the logic across two or more files like so:
      ...
      MyApp.StateA = Ember.State.extend({
        substatesAreConcurrent: true,
      ...
      MyApp.StateB = Ember.State.extend({
        substatesAreConcurrent: true,
      ...
      MyApp.Statechart = Ember.Object.extend(Ember.StatechartManager, {
        rootState: Ember.State.extend({
          initialSubstate: 'stateA',
          stateA: Ember.State.plugin('MyApp.StateA'),
          stateB: Ember.State.plugin('MyApp.StateB')
        })
      });
...
Ember.StatechartManager = /** @scope Ember.StatechartManager.prototype */{
  ...
  */
  ...
  /** 
    ...
    this statechart. The class assigned must derive from Ember.State. 
  ...
  */
  ...
  /**
    ...
    @property {Ember.StatechartMonitor}
  */
  monitor: null,
  ...
  */
  ...
  destroy: function() {
    ...
    Ember.removeObserver(this, traceKey, this, '_statechartTraceDidChange');
  ...
  */
  initStatechart: function() {
    ...
    if (get(this, 'monitorIsActive')) {
      set(this, 'monitor', Ember.StatechartMonitor.create({ statechart: this }));
    }
    ...
    else if (Ember.typeOf(rootState) === "function" && rootState.statePlugin) {
      rootState = rootState.apply(this);
    }
    
    if (!(Ember.State.detect(rootState) && rootState.isClass)) {
    ...
    rootState = this.createRootState(rootState, { 
      statechart: this, 
      stateName: Ember.ROOT_STATE_NAME 
    });
    
    set(this, 'rootState', rootState);
    rootState.initState();
    
    if (Ember.EmptyState.detect(get(rootState, 'initialSubstate'))) {
      msg = "Unable to initialize statechart. Root state must have an initial substate explicilty defined";
      this.statechartLogError(msg);
      throw msg;
    }
    
    if (!Ember.empty(get(this, 'initialState'))) {
  ...
  */
  doesContainState: function(value) {
    return !Ember.none(this.getState(value));
  },
  ...
  */
  ...
  /**
    ...
    where <state> is either a Ember.State object or a string and <hash> is a regular JS hash object.
    
    @param state {Ember.State|String} the state to go to (may not be the final state in the transition process)
    @param fromCurrentState {Ember.State|String} Optional. The current state to start the transition process from.
  ...
  */
  gotoState: function(state, fromCurrentState, useHistory, context) {
    ...
    if (!Ember.none(fromCurrentState)) {
      ...
      if (Ember.none(fromCurrentState) || !get(fromCurrentState, 'isCurrentState')) {
  ...
  */
  ...
  _executeGotoStateActions: function(gotoState, actions, marker, context) {
    ...
    for (; marker < len; marker += 1) {
      ...
      //
      if (actionResult instanceof Ember.Async) {
  ...
  /**
    ...
    @param state {Ember.State} the state whose enterState method is to be invoked
  ...
  */
  ...
  /**
    ...
    @param state {Ember.State|String} the state to go to and follow it's history state
    @param fromCurrentState {Ember.State|String} Optional. the current state to start the state transition process from
  ...
  */
  ...
  /**
    ...
    @returns {Ember.Responder} the responder that handled it or null
  */
  ...
  */
  _traverseStatesToExit: function(state, exitStatePath, stopState, gotoStateActions) {    
    ...
    gotoStateActions.push({ action: Ember.EXIT_STATE, state: state });
  ...
  */
  _traverseStatesToEnter: function(state, enterStatePath, pivotState, useHistory, gotoStateActions) {
    ...
    // other criteria
      ...
      var gotoStateAction = { action: Ember.ENTER_STATE, state: state, currentState: false };
      ...
      // State has an initial substate to enter
        ...
        if (initialSubstate instanceof Ember.HistoryState) {
    ...
    // Still have an explicit enter path to follow, so keep moving through the path.
      ...
      gotoStateActions.push({ action: Ember.ENTER_STATE, state: state });
  ...
  */
  respondsTo: function(action) {
    ...
    return Ember.typeOf(this[action]) === "function";   
  },
  ...
  */
  tryToPerform: function(action, arg1, arg2) {
    if (this.respondsTo(action)) {
      if (Ember.typeOf(this[action]) === "function") return (this[action](arg1, arg2) !== false);
  ...
  */
  invokeStateMethod: function(methodName, args, func) {
    ...
    var len = args.length, 
        ...
        callback = Ember.typeOf(arg) === "function" ? args.pop() : null,
    ...
    for (; i < len; i += 1) {
      ...
      while (state) {
        ...
        if (Ember.typeOf(method) === "function" && !method.isActionHandler) {
  ...
  */
  ...
  _monitorIsActiveDidChange: function() {
    if (get(this, 'monitorIsActive') && Ember.none(get(this, 'monitor'))) {
      set(this, 'monitor', Ember.StatechartMonitor.create());
    }
  ...
  */
  ...
  /** @private 
    ...
    it based on properties found on this state that derive from a Ember.State class. For the
    root state to be successfully built, the following much be met:
    
     - The rootStateExample property must be defined with a class that derives from Ember.State
  ...
  */
  _constructRootStateClass: function() {
    ...
    if (Ember.typeOf(rsExample) === "function" && rsExample.statePlugin) {
      rsExample = rsExample.apply(this);
    }

    if (!(Ember.State.detect(rsExample) && rsExample.isClass)) {
      this._logStatechartCreationError("Invalid root state example");
      return null;
    }
    
    if (statesAreConcurrent && !Ember.empty(initialState)) {
    ...
    } else if (Ember.typeOf(initialState) === "string") {
  ...
  _logStatechartCreationError: function(msg) {
    Ember.Logger.error("Unable to create statechart for %@: %@.".fmt(this, msg));
  },
  ...
  */
  statechartLogTrace: function(msg) {
    Ember.Logger.info("%@: %@".fmt(get(this, 'statechartLogPrefix'), msg));
  },
  ...
  */
  statechartLogError: function(msg) {
    Ember.Logger.error("ERROR %@: %@".fmt(get(this, 'statechartLogPrefix'), msg));
  },
  ...
  */
  statechartLogWarning: function(msg) {
    ...
    Ember.Logger.warn("WARN %@: %@".fmt(get(this, 'statechartLogPrefix'), msg));
  },
  ...
  statechartLogPrefix: function() {
    ...
    if (Ember.empty(name)) prefix = "%@<%@>".fmt(className, Ember.guidFor(this));
    else prefix = "%@<%@, %@>".fmt(className, name, Ember.guidFor(this));
...
*/
Ember.ROOT_STATE_NAME = "__ROOT_STATE__";
...
/**
  A Startchart class. 
*/
Ember.Statechart = Ember.Object.extend(Ember.StatechartManager, {
