...
module("Ember.ManyArray core methods", {
  setup: function() {
...
test("changing storeIds length should change length of rec array also", function() {
  ...
  storeIds.pushObject(Ember.Store.generateStoreKey()); // change length
...
test("reading a record not loaded in store should trigger retrieveRecord", function() {
  ...
  MyApp.store.removeDataHash(storeKey, Ember.Record.EMPTY);
  MyApp.store.retrieveRecord = function() { callCount++; };
  
  var rec = recs.objectAt(0);
  equals(MyApp.store.readStatus(rec), Ember.Record.EMPTY, 'precond - storeKey must not be loaded');
...
test("waits for id if it's not yet attached to object", function() {
  ...
  Ember.run(function() {
    Ember.Store.replaceIdFor(storeKey, 100);
