...
module("Ember.NestedStore#removeDataHash", {
  setup: function() {
...
function testRemoveDataHash() {
  ...
  // perform test
  equals(store.removeDataHash(storeKey, Ember.Record.DESTROYED_CLEAN), store, 'should return receiver');
  
  // verify
  equals(store.storeKeyEditState(storeKey), Ember.Store.LOCKED, 'new edit state should be locked');
  
  equals(store.readDataHash(storeKey), null, 'should have NO json data');
  equals(store.readStatus(storeKey), Ember.Record.DESTROYED_CLEAN, 'should have new status');
...
test("remove a non-existing hash", function() {
  ...
  // verify change
  equals(store.storeKeyEditState(storeKey), Ember.Store.LOCKED, 'new status should be locked');
  equals(store.readDataHash(storeKey), null, 'should still be null');
  equals(store.readStatus(storeKey), Ember.Record.DESTROYED_CLEAN, 'should have new record status');
});
...
test("change should propogate to child if child edit state = INHERITED", function() {
  ...
  // perform change
  store.removeDataHash(storeKey, Ember.Record.DESTROYED_CLEAN);
  ...
  equals(child.readStatus(storeKey), Ember.Record.DESTROYED_CLEAN, 'child should pick up new status');
