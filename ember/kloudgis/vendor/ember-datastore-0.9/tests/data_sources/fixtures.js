...
module("Ember.FixturesDataSource", {
  setup: function() {
    Ember.run.begin();
    
    Sample = Ember.Object.create();
    Sample.File = Ember.Record.extend({ test:'hello'});
...
test("Update and commit a record", function() {
  ...
  equals(get(rec, 'status'), Ember.Record.READY_CLEAN, "Status should be READY_CLEAN because no changes have been made");

  set(rec, 'name', 'foo');
  equals(get(rec, 'status'), Ember.Record.READY_DIRTY, "Status should be READY_DIRTY after changing name "+rec.toString());

  store.commitRecords();
  equals(store.readStatus(storeKey), Ember.Record.READY_CLEAN, "Status in store should be READY_CLEAN after save");
  equals(get(rec, 'status'), Ember.Record.READY_CLEAN, "Status in record should be READY_CLEAN after save");
