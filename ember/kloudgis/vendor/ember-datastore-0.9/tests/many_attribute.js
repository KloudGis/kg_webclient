...
module("Ember.ManyAttribute core methods", {
  setup: function() {
    Ember.run.begin();
    ...
    MyApp.Foo = Ember.Record.extend({
      ...
      otherName: Ember.Record.attr(String, { key: "firstName" }),
      
      // test mapping Date
      date: Ember.Record.attr(Date),
    ...
    MyApp.Bar = Ember.Record.extend({
      ...
      // test many-to-many
      fooToMany: Ember.Record.toMany('MyApp.Foo', {
        inverse: 'barToMany', isMaster: NO
      }),
      
      // test many-to-one
      fooToOne: Ember.Record.toOne('MyApp.Foo', {
...
test("getting toMany relation should not change record state", function() {
  equals(get(rec3, 'status'), Ember.Record.READY_CLEAN, 'precond - status should be READY_CLEAN');
  
  var recs = get(rec3, 'fooMany');
  ok(recs, 'get(rec3, fooMany) should return records');
  equals(get(rec3, 'status'), Ember.Record.READY_CLEAN, 'getting toMany should not change state');
});
...
test("modifying a toMany array should mark the record as changed", function() {
  var recs = get(rec3, 'fooMany');
  equals(get(rec3, 'status'), Ember.Record.READY_CLEAN, 'precond - rec3.status should be READY_CLEAN');
  ...
  equals(get(rec3, 'status'), Ember.Record.READY_DIRTY, 'record status should have changed to dirty');
...
test("modifying a toMany array within a nested store", function() {
  ...
  Ember.run.end(); // allow notifications to process, if there were any...
  
  same(parentFooMany.getEach('storeKey'), expected, 'parent.fooMany should not have changed yet');
  equals(get(rec3, 'status'), Ember.Record.READY_CLEAN, 'parent rec3 should still be READY_CLEAN');
  ...
  equals(get(rec3, 'status'), Ember.Record.READY_DIRTY, 'parent rec3 should now be READY_DIRTY');
...
function checkAllClean() {
  Array.prototype.slice.call(arguments).forEach(function(r) {
    equals(get(r, 'status'), Ember.Record.READY_CLEAN, 'PRECOND - %@.status should be READY_CLEAN'.fmt(get(r, 'id')));
  }, this);
}
...
test("removing a record from a many-to-many", function() {
  ...
  equals(get(foo1, 'status'), Ember.Record.READY_DIRTY, 'foo1.status should be READY_DIRTY');
  equals(get(bar1, 'status'), Ember.Record.READY_CLEAN, 'bar1.status should be READY_CLEAN');
...
test("adding a record to a many-to-many; bar side", function() {
  ...
  equals(get(foo2, 'status'), Ember.Record.READY_DIRTY, 'foo1.status should be READY_DIRTY');
  equals(get(bar1, 'status'), Ember.Record.READY_CLEAN, 'bar1.status should be READY_CLEAN');
});
...
test("adding a record to a many-to-many; foo side", function() {
  ...
  equals(get(foo2, 'status'), Ember.Record.READY_DIRTY, 'foo1.status should be READY_DIRTY');
  equals(get(bar1, 'status'), Ember.Record.READY_CLEAN, 'bar3.status should be READY_CLEAN');
});
...
test("removing a record from a one-to-many", function() {
  ...
  equals(get(foo1, 'status'), Ember.Record.READY_CLEAN, 'foo1.status should be READY_CLEAN');
  equals(get(bar1, 'status'), Ember.Record.READY_DIRTY, 'bar1.status should be READY_DIRTY');
...
test("add a record to a one-to-many; many-side", function() {
  ...
  equals(get(foo1, 'status'), Ember.Record.READY_CLEAN, 'foo1.status should be READY_CLEAN');
  equals(get(bar3, 'status'), Ember.Record.READY_DIRTY, 'bar3.status should be READY_DIRTY');
