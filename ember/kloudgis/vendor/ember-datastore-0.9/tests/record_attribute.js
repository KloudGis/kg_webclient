...
module("Ember.RecordAttribute core methods", {
  setup: function() {
    ...
    MyApp.Foo = Ember.Record.extend({
      ...
      nonIsoDate: Ember.Record.attr(Date, { useIsoDate: false }),

      // test Ember.DateTimes
      dateTime: Ember.Record.attr(Ember.DateTime),
      
      // test Array
      anArray: Ember.Record.attr(Array),
      
      // test Object
      anObject: Ember.Record.attr(Object),
                                 
      // test Number
      aNumber: Ember.Record.attr(Number),
      ...
      // used to test default value
      defaultComputedValue: Ember.Record.attr(Number, {
      ...
      // test readONly
      readOnly: Ember.Record.attr(String, { isEditable: NO })
      
    });

    MyApp.Bar = Ember.Record.extend({
      parent: Ember.Record.toOne('MyApp.Foo', { aggregate: YES }),
      relatedMany: Ember.Record.toMany('MyApp.Foo', { aggregate: YES })
    });
    ...
    storeKeys = MyApp.store.loadRecords(MyApp.Foo, [
      ...
      { 
        ...
        dateTime: Ember.DateTime.create(1235939425000),
        anArray: ['one', 'two', 'three'],
        anObject: { 'key1': 'value1', 'key2': 'value2' },
        aNumber: '123'
      }
...
test("reading dateTime should parse ISO date", function() {
  ...
  equals(Ember.getPath(rec, 'dateTime.milliseconds'), ms, 'should have parsed Date properly');
  equals(Ember.getPath(rec2, 'dateTime.milliseconds'), ms, 'should have parsed String properly');
  equals(Ember.getPath(rec3, 'dateTime.milliseconds'), ms, 'should have parsed Ember.DateTime properly');
});
...
test("writing an attribute should make relationship aggregate dirty" ,function() {
  equals(get(bar, 'status'), Ember.Record.READY_CLEAN, "precond - bar should be READY_CLEAN");
  equals(get(rec2, 'status'), Ember.Record.READY_CLEAN, "precond - rec2 should be READY_CLEAN");
  
  set(bar, 'city', 'Oslo');
  get(bar, 'store').flush();
  
  equals(get(rec2, 'status'), Ember.Record.READY_DIRTY, "foo2 should be READY_DIRTY");
});
...
test("writing an attribute should make many relationship aggregate dirty" ,function() {
  ...
  equals(get(rec, 'status'), Ember.Record.READY_DIRTY, "foo1 should be READY_DIRTY");
  equals(get(rec2, 'status'), Ember.Record.READY_DIRTY, "foo2 should be READY_DIRTY");
});
