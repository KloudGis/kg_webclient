/**
 * Nested Records and the Data Store(Ember.Record) Unit Test
 *
 * @author Evin Grano
 */
...
var initModels = function(){
  NestedRecord.Directory = Ember.Record.extend({
    /** Child Record Namespace */
    nestedRecordNamespace: NestedRecord,
    name: Ember.Record.attr(String),
    contents: Ember.Record.toMany('Ember.Record', { isNested: true })
  });
  
  NestedRecord.File = Ember.Record.extend({
    name: Ember.Record.attr(String)
  });
...
test("Proper Initialization",function() {
  ...
  ok((first instanceof  Ember.Record), "first record is a kind of a Ember.Record Object");
  ...
  ok((second instanceof  Ember.Record), "second record is a kind of a Ember.Record Object");
...
test("Proper Status",function() {
  ...
  // First
  first = store.materializeRecord(storeKeys[0]);
  equals(get(first, 'status'), Ember.Record.READY_CLEAN, 'first record has a READY_CLEAN State');
  
  // Second
  second = store.materializeRecord(storeKeys[1]);
  equals(get(second, 'status'), Ember.Record.READY_CLEAN, 'second record has a READY_CLEAN State');
});
...
test("Can Push onto child array",function() {
  ...
  contents.forEach(function(f){
    ok((f instanceof NestedRecord.File), "should be a NestedRecord.File");
    ok(get(f, 'name'), "should have a name property");
    equals(get(f, 'status'), Ember.Record.READY_DIRTY, 'second record has a READY_CLEAN State');
...
test("Use in Nested Store", function(){
  ...
  equals(get(dir, 'status'), Ember.Record.READY_DIRTY, 'Base > Directory id:1 has a READY_DIRTY State');
  equals(get(file, 'status'), Ember.Record.READY_DIRTY, 'Base > File id:1 has a READY_DIRTY State');
