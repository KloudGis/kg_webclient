/**
 * Nested Record Array (Ember.ChildRecord) Unit Test
 *
 * @author Evin Grano
 */
...
var initModels = function(){
  NestedRecord.Group = Ember.Record.extend({
    /** Child Record Namespace */
    nestedRecordNamespace: NestedRecord,

    name: Ember.Record.attr(String),
    people: Ember.Record.toMany('NestedRecord.Person', { nested: true })
  });
  ...
  NestedRecord.Person = Ember.Record.extend({
    ...
    name: Ember.Record.attr(String),
    addresses: Ember.Record.toMany('NestedRecord.Address', { nested: true })
  });
...
// ..........................................................
// Basic Ember.ParentRecord with an Array of Children
// 
module("Complex Ember.Record: Parent > Array of Children > Array of Children", {
...
test("Basic Read, Testing the First Child Array", function() {
  ...
  ok((ppl instanceof Ember.ChildArray), "check that get() creates an actual instance of a Ember.ChildArray");
  equals(get(ppl, 'length'), 3, "check that the length of the array of child records is 3");
  p = ppl.objectAt(0);
  ok((p instanceof  Ember.Record), "check that first ChildRecord from the get() creates an actual instance that is a kind of a Ember.Record Object");
  ...
  ok(!(pplAttr instanceof Ember.ChildArray), "check that readAttribute() does not create an actual instance of a Ember.ChildArray");
  ...
  equals(Ember.guidFor(pDup), Ember.guidFor(p), "check to see if the guid are the same");
...
test("Basic Read, Testing the Second Child Array", function() {
  ...
  ok((addrs instanceof Ember.ChildArray), "check that get() creates an actual instance of a Ember.ChildArray");
  equals(get(addrs, 'length'), 2, "check that the length of the array of child records is 2");
  a = addrs.objectAt(0);
  ok((a instanceof  Ember.Record), "check that first ChildRecord from the get() creates an actual instance that is a kind of a Ember.Record Object");
  ...
  ok(!(addrsAttr instanceof Ember.ChildArray), "check that readAttribute() does not create an actual instance of a Ember.ChildArray");
  ...
  pplAttr = testParent.readAttribute('people');
  ok(!(pplAttr[0].addresses instanceof Ember.ChildArray), "check from the Group (parent Record) that readAttribute() does not create an actual instance of a Ember.ChildArray");
  ...
  equals(Ember.guidFor(aDup), Ember.guidFor(a), "check to see if the guid are the same");
...
test("Basic Write: Testing the First Child Array", function() {
  ...
  equals(get(testParent, 'nothing'), 'nothing', "set should change non-existent property to a new property");
   ...
   ok((pAddrs instanceof  Ember.ChildArray), "check to see that the set('addresses') has returned a Ember.ChildArray");
   ...
   ok(get(p, 'status') & Ember.Record.DIRTY, 'check that the person (child record) is dirty');
   ok(get(testParent, 'status') & Ember.Record.DIRTY, 'check that the group (parent record) is dirty');
});
...
test("Basic Write: Testing the Second Child Array", function() {
  ...
  ok(get(a, 'status') & Ember.Record.DIRTY, 'check that the address (child record) is dirty');
  ok(get(p, 'status') & Ember.Record.DIRTY, 'check that the person (child record) is dirty');
  ok(get(testParent, 'status') & Ember.Record.DIRTY, 'check that the group (parent record) is dirty');
...
test("Basic Array Functionality: pushObject", function() {   
  ...
  p = ppl.objectAt(3);
  ok((p instanceof  Ember.Record), "check that newly added ChildRecord creates an actual instance that is a kind of a Ember.Record Object");
