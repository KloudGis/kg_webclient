/**
 * Nested Record Array of Ember.Records Unit Test
 *
 * @author Evin Grano
 */
...
var initModels = function(){
  NestedRecord.ParentRecordTest = Ember.Record.extend({
    ...
    elements: Ember.Record.toMany('Ember.Record', { nested: true })
  });

  NestedRecord.ChildRecordTest1 = Ember.Record.extend({
    name: Ember.Record.attr(String),
    value: Ember.Record.attr(String)
  });
  
  NestedRecord.ChildRecordTest2 = Ember.Record.extend({
     name: Ember.Record.attr(String),
     info: Ember.Record.attr(String),
     value: Ember.Record.attr(String)
   });
};

// ..........................................................
// Basic Ember.Record with an Array of Children
// 
module("Basic Ember.Record w/ a Parent > Array of Children", {
...
test("Basic Read", function() {
  ...
  ok((arrayOfCRs instanceof Ember.ChildArray), "check that get() creates an actual instance of a Ember.ChildArray");
  equals(get(arrayOfCRs, 'length'), 4, "check that the length of the array of child records is 4");
  var cr = arrayOfCRs.objectAt(0);
  ok((cr instanceof  Ember.Record), "check that first ChildRecord from the get() creates an actual instance that is a kind of a Ember.Record Object");
  ...
  ok(!(parentArray instanceof Ember.ChildArray), "check that get() creates an actual instance of a Ember.ChildArray");
  ...
  equals(Ember.guidFor(cr), Ember.guidFor(sameCR), "check to see if the guid are the same");
...
test("Basic Write", function() {
  ...
  ok((newArray instanceof Ember.ChildArray), "check that get() creates an actual instance of a Ember.ChildArray");
...
test("Basic Array Functionality: pushObject w/ HASH", function() {   
  ...
  ok((cr instanceof  Ember.Record), "check that newly added ChildRecord creates an actual instance that is a kind of a Ember.Record Object");
...
test("Create Parent with Broken Child Array", function(){
  ...
  ok (!Ember.none(elements), "elements should be something");
  var isChildRecordArrays = elements instanceof Ember.ChildArray;
