...
module("Ember.Store#commitRecord", {
  setup: function() {

    ds = Ember.DataSource.create({
    ...
    store = Ember.Store.create().from(ds);
    ...
    store.writeDataHash(storeKey3, json3, Ember.Record.READY_DIRTY);
    storeKey4 = Ember.Store.generateStoreKey();
    store.writeDataHash(storeKey4, json4, Ember.Record.DESTROYED_DIRTY);
    storeKey5 = Ember.Store.generateStoreKey();
    store.writeDataHash(storeKey5, json5, Ember.Record.READY_EMPTY);
    storeKey6 = Ember.Store.generateStoreKey();
    store.writeDataHash(storeKey6, json6, Ember.Record.READY_ERROR);
    storeKey7 = Ember.Store.generateStoreKey();
    store.writeDataHash(storeKey7, json7, Ember.Record.READY_DESTROYED_CLEAN);
    Ember.run.end();
  }
});
...
test("Confirm that all the states are switched as expected after running commitRecord", function() {
  ...
  equals(status, Ember.Record.READY_CLEAN, "the status shouldn't have changed. It should be READY_CLEAN ");
  
  store.commitRecord(undefined, undefined, storeKey2);
  status = store.readStatus( storeKey2);
  equals(status, Ember.Record.BUSY_CREATING, "the status should be Ember.Record.BUSY_CREATING");

  store.commitRecord(undefined, undefined, storeKey3);
  status = store.readStatus( storeKey3);
  equals(status, Ember.Record.BUSY_COMMITTING, "the status should be Ember.Record.BUSY_COMMITTING");
  
  store.dataSourceDidComplete(storeKey3);
  status = store.readStatus( storeKey3);
  equals(status, Ember.Record.READY_CLEAN, "the status should be Ember.Record.READY_CLEAN");
  
  store.commitRecord(undefined, undefined, storeKey4);
  status = store.readStatus( storeKey4);
  equals(status, Ember.Record.BUSY_DESTROYING, "the status should be Ember.Record.BUSY_DESTROYING");
  ...
  equals(msg, Ember.Record.NOT_FOUND_ERROR.message, "commitRecord should throw the following error");
...
test("calling commitRecords() without explicit storeKeys", function() {
  ...
  equals(st, Ember.Record.READY_CLEAN, "storeKey1 - the status shouldn't have changed. It should be READY_CLEAN ");
  
  st = store.readStatus( storeKey2);
  equals(st, Ember.Record.BUSY_CREATING, "storeKey2 - the status should be Ember.Record.BUSY_CREATING");

  st = store.readStatus( storeKey3);
  equals(st, Ember.Record.BUSY_COMMITTING, "storeKey3 - the status should be Ember.Record.BUSY_COMMITTING");
  
  st = store.readStatus( storeKey4);
  equals(st, Ember.Record.BUSY_DESTROYING, "storeKey4 - the status should be Ember.Record.BUSY_DESTROYING");
