...
module("Ember.RecordAttribute core methods", {
  setup: function() {
    ...
    MyApp.Foo = Ember.Record.extend({
      ...
      relatedTo: Ember.Record.toOne('MyApp.Foo'),

      // test non-isEditable toOne relationships
      readOnlyRelatedTo: Ember.Record.toOne('MyApp.Bar', {
        isEditable: NO
      }),
      ...
      bar: Ember.Record.toOne('MyApp.Bar', { inverse: 'foo' }),
      
      barKeyed: Ember.Record.toOne('MyApp.Bar', { key: 'barId' })
      
    });
    
    MyApp.Bar = Ember.Record.extend({
      foo: Ember.Record.toOne('MyApp.Foo', { inverse: 'bar', isMaster: NO })
    });
...
test("clearing a toOne relationship with an inverse - foo isMaster", function() {
  ...
  equals(get(rec, 'status'), Ember.Record.READY_CLEAN, 'precond - foo1.status should be READY_CLEAN');
  equals(get(bar, 'status'), Ember.Record.READY_CLEAN, 'precond - bar1.status should be READY_CLEAN');
  ...
  equals(get(rec, 'status'), Ember.Record.READY_DIRTY, 'foo1.status should be READY_DIRTY');
  equals(get(bar, 'status'), Ember.Record.READY_CLEAN, 'bar1.status should be READY_CLEAN');
...
test("modifying a toOne relationship with an inverse from null", function() {
  ...
  equals(get(rec, 'status'), Ember.Record.READY_DIRTY, 'foo1.status should be READY_DIRTY');
  equals(get(rec2, 'status'), Ember.Record.READY_DIRTY, 'foo1.status should be READY_DIRTY');
  equals(get(bar, 'status'), Ember.Record.READY_CLEAN, 'bar1.status should be READY_CLEAN');
...
test("modifying a toOne relationship with an inverse from other", function() {
  ...
  equals(get(bar2, 'status'), Ember.Record.READY_CLEAN, 'bar1.status should be READY_CLEAN');
...
test("isEditable NO should not allow editing", function() {
  ...
  equals(get(rec5, 'status'), Ember.Record.READY_CLEAN, 'precond - foo5 should be READY_CLEAN');
  
  set(rec5, 'readOnlyRelatedTo', bar2);
  
  equals(get(rec5, 'readOnlyRelatedTo'), bar1, 'should still find bar1 after setting');
  equals(get(rec5, 'status'), Ember.Record.READY_CLEAN, 'foo5 status is still READY_CLEAN');
});
