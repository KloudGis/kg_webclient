...
module("Ember.IndexSet#add", {
  setup: function() {
    set = new Ember.IndexSet();
  }
});
...
test("add with index set should add indexes in set", function() {
  var ranges = Ember.IndexSet.create().add(2,2).add(10,2);
  set.add(ranges);
...
test("adding a range should trigger an observer notification", function() {
  var callCnt = 0;
  Ember.addObserver(set, '[]', function() { callCnt++; });
  set.add(10,10);
...
test("appending a range to end should merge into last range", function() {
  set = Ember.IndexSet.create(2).add(3);
  ...
  set = Ember.IndexSet.create(2000, 1000).add(3000, 1000);
