...
module("Ember.IndexSet#without", {
  setup: function() {
    set = Ember.IndexSet.create(1,9);
  }
});
...
test("should return set with multiple ranges removed", function() {
  ret = set.without(Ember.IndexSet.create(2,2).add(6,2));
  ok(ret !== set, 'is not same instance');
  same(iter(ret), [1,4,5,8,9]);
});

test("using without should properly hint returned index set", function() {
  var set = Ember.IndexSet.create(10000,5),
      set2 = Ember.IndexSet.create(10000),
      actual = set.without(set2),
      loc = Ember.IndexSet.HINT_SIZE;
